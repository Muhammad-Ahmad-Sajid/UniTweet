...
class FeedBaseScreen(tk.Frame):
    ...
    def render_tweets(self, tweets, clear=True):
        ...
        for t in tweets:
            ...
            # Content
            ttk.Label(card, text=t['content'], style="Card.TLabel", wraplength=700).pack(fill="x", pady=(10, 5), anchor="w")

            img_data = db.get_tweet_image(t['tweet_id'])
            if img_data and img_data.get('image'):
                try:
                    ...
                    if not hasattr(self, 'tweet_images'): self.tweet_images = []
                    self.tweet_images.append(img_tk)
                    tk.Label(card, image=img_tk, bg=CARD_BG).pack(anchor="w", pady=(0, 15))
                except Exception as e:
                    print(f"Error loading image for tweet {t['tweet_id']}: {e}")
                    tk.Frame(card, height=10, bg=CARD_BG).pack()
            else:
                tk.Frame(card, height=10, bg=CARD_BG).pack()
...
class ComposeTweetScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.img_frame = tk.Frame(self, bg=BG_COLOR)
        self.img_frame.pack(fill="x", padx=30, pady=5)
        
        HoverButton(self.img_frame, text="📎 Attach Image (optional)", command=self.attach_image, bg=CARD_BG, fg=TEXT_COLOR, 
                    activebackground="#22303c", font=("Segoe UI", 10), relief="flat", cursor="hand2").pack(side="left")
        
        self.img_lbl = tk.Label(self.img_frame, text="", bg=BG_COLOR, fg=DIM_TEXT, font=("Segoe UI", 10))
        self.img_lbl.pack(side="left", padx=10)
        
        self.remove_img_btn = HoverButton(self.img_frame, text="✕ Remove", command=self.remove_image, bg=BG_COLOR, fg=DANGER, 
                    activebackground=BG_COLOR, font=("Segoe UI", 10), relief="flat", cursor="hand2")
                    
        self.post_error_lbl = tk.Label(self, text="", bg=BG_COLOR, fg=DANGER, font=("Segoe UI", 10))
        self.post_error_lbl.pack(fill="x", padx=30, pady=0)

        bot = tk.Frame(self, bg=BG_COLOR)
        bot.pack(fill="x", padx=30, pady=10)
    ...
    def on_show(self):
        ...
        self.remove_image()
        if hasattr(self, 'post_error_lbl'):
            self.post_error_lbl.config(text="")
        self.update_count()
    ...
    def attach_image(self):
        ...
        if filepath:
            ...
            if file_size > 5 * 1024 * 1024:
                self.post_error_lbl.config(text="Error: Image exceeds 5MB limit.")
                return
            self.selected_image_path = filepath
            self.img_lbl.config(text=f"Selected: {os.path.basename(filepath)}")
    ...
    def post(self):
        ...
        if not content or len(content) > 280:
            self.post_error_lbl.config(text="Error: Tweet text must be 1-280 characters.")
            return
        ...
        try:
            if self.selected_image_path:
                ...
                db.create_tweet_with_image(session["user"]["reg_no"], content, image_bytes, filename)
            else:
                db.create_tweet(session["user"]["reg_no"], content)
                
            self.controller.show_frame(HomeFeedScreen)
        except Exception as e:
            self.post_error_lbl.config(text=f"Error posting tweet: {str(e)[:50]}")
...
class ProfileScreen(FeedBaseScreen):
    ...
    def on_show(self, target_id=None):
        ...
        # Avatar Frame
        avatar_frame = tk.Frame(header_card, bg=CARD_BG)
        avatar_frame.pack(anchor="w", pady=(0, 10))
        
        avatar_data = db.get_avatar(self.target_id)
        if avatar_data and avatar_data.get('avatar'):
            try:
                from PIL import Image, ImageTk
                import io
                image = Image.open(io.BytesIO(avatar_data['avatar']))
                image = image.resize((100, 100), Image.Resampling.LANCZOS)
                self.avatar_image_tk = ImageTk.PhotoImage(image)
                tk.Label(avatar_frame, image=self.avatar_image_tk, bg=CARD_BG).pack(side="left")
            except Exception as e:
                print("Error loading avatar:", e)
                tk.Label(avatar_frame, text="No Photo", bg="gray", fg="white", width=14, height=6).pack(side="left")
        else:
            tk.Label(avatar_frame, text="No Photo", bg="gray", fg="white", width=14, height=6).pack(side="left")

        # Upload Button
        if self.target_id == session["user"]["reg_no"]:
            HoverButton(avatar_frame, text="📷 Upload Picture", command=self.upload_avatar_action,
                        bg=CARD_BG, fg=TEXT_COLOR, font=("Segoe UI", 10, "bold"), relief="solid", borderwidth=1, cursor="hand2").pack(side="left", padx=15)
            self.avatar_error_lbl = tk.Label(avatar_frame, text="", bg=CARD_BG, fg=SUCCESS)
            self.avatar_error_lbl.pack(side="left", padx=10)
    ...
    def upload_avatar_action(self):
        ...
        file_size = os.path.getsize(filepath)
        if file_size > 5 * 1024 * 1024:
            self.avatar_error_lbl.config(text="Error: Image > 5MB", fg=DANGER)
            return
        ...
        try:
            ...
            db.upload_avatar(self.target_id, image_bytes, filename)
            self.avatar_error_lbl.config(text="Profile picture updated!", fg=SUCCESS)
            self.on_show(self.target_id)
        except Exception as e:
            self.avatar_error_lbl.config(text=f"Error: {str(e)[:20]}", fg=DANGER)
...
class TweetDetailScreen(tk.Frame):
    ...
    def on_show(self, tweet_id=None):
        ...
        ttk.Label(card, text=t['content'], font=("Segoe UI", 18), background=CARD_BG, foreground=TEXT_COLOR, wraplength=700).pack(anchor="w", pady=10)

        img_data = db.get_tweet_image(self.tweet_id)
        if img_data and img_data.get('image'):
            try:
                from PIL import Image, ImageTk
                import io
                ...
                tk.Label(card, image=self.detail_image, bg=CARD_BG).pack(anchor="w", pady=(0, 20))
            except Exception as e:
